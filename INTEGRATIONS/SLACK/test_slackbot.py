...
OPENAI_API_BASE_URL = get_env('OPENAI_API_BASE_URL', None)
OPENAI_API_DEPLOYMENT = get_env('OPENAI_API_DEPLOYMENT', None)
OPENAI_API_VERSION = get_env('OPENAI_API_VERSION', None)
...
def handle_prompt(prompt, channel, thread_ts=None, direct_message=False):
    ...
    if prompt.lower().startswith('image:'):
        ...
        try:
            ...
            url = f'{OPENAI_API_BASE_URL}/{OPENAI_API_DEPLOYMENT}?api-version={OPENAI_API_VERSION}'
            ...
            log(f'ChatGPT response error: {str(e)}', error=True)
            # Reply with error message
            client.chat_postMessage(channel=channel, thread_ts=thread_ts, text=str(e))
            return
